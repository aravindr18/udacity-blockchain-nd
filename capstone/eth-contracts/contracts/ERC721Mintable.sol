...
contract Ownable {
    ...
    event OwnershipTransferred(address ownerAddress);
    ...
    modifier requireNonZeroAddress(address newOwner){
        require(newOwner != address(0), "invalid address! ownership transfer not possible!");
        _;
    }
    //  4) fill out the transferOwnership function
    //  5) create an event that emits anytime ownerShip is transfered (including in the constructor)
    function transferOwnership(address newOwner) public onlyOwner requireNonZeroAddress(newOwner) {
...
function pause(bool status) public onlyOwner{
    _paused = status;
    // is status = true then the caller wants to pause the contract
    if(status){
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != _tokenOwner[tokenId], "the address is the owner of the token already");
        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(isApprovedForAll(getOwner(), msg.sender) || msg.sender == _tokenOwner[tokenId], "approval failed");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId), 'require token to exist before setting URI');
...
contract MyRealEstateToken is ERC721Metadata("MyRealEstate Token", "MyREST", 
